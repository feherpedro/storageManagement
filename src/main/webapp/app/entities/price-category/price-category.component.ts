...
@Component({
    selector: 'jhi-price-category',
    templateUrl: './price-category.component.html'
})
export class PriceCategoryComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.priceCategoryService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<PriceCategory[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/price-category'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/price-category', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PriceCategory) {
        return item.id;
    }
